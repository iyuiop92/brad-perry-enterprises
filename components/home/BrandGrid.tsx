const brands = [
  ...
  { name: 'SuperWatches', initials: 'SW', color: '#f59e0b' },
        ...
        {brands.map((brand) => (
          <div key={brand.name} className="flex flex-col items-center gap-3 group">
            <div className="w-16 h-16 flex items-center justify-center transition-transform duration-200 group-hover:scale-110">
              <span className="text-sm font-[800]" style={{ color: brand.color }}>
                {brand.initials}
              </span>
            </div>
            <span className="text-white text-xs text-center leading-tight group-hover:text-white transition-colors">
              {brand.name}
            </span>
          </div>
        ))}
